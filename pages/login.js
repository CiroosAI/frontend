...
export default function Login() {
  ...
  // Fetch latest settings from server (to get maintenance/closed_register and name)
  (async () => {
    try {
      const data = await getInfo();
      if (data && data.success && data.data) {
        const app = data.data;
        // Update local storage application if name present
        if (app.name) {
      ...
      // ignore fetch errors here; keep existing applicationData
    }
  })();
}, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result && result.success === true) {
        ...
        // Token and user storage are handled inside `loginUser` (sessionStorage/cookie).
        // Other parts of the app listen for the 'user-token-changed' event to update state.
  ...
  return (
    <>
      <Head>
  <title>{applicationData?.name || 'Vla Devs'} | Login</title>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 animate-slideUp">
              ...
              <button
                ...
                className={`w-full font-bold py-4 px-6 rounded-2xl transition-all duration-300 flex items-center justify-center gap-3 shadow-lg border border-purple-400/30 relative overflow-hidden ${
                  // disable button when maintenance or form invalid
