...
import { ArrowLeft, Plus, Edit3, Trash2, AlertTriangle } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#0A0A0A] pb-32 relative overflow-hidden">
      ...
      <div className="max-w-md mx-auto p-4 relative z-10">
        ...
        <div className="bg-gradient-to-br from-[#1A1A1A] to-[#0F0F0F] rounded-3xl p-5 border border-white/10 shadow-2xl mb-6 mt-2">
          <div className="flex items-center gap-4 mb-5">
            <div className="w-12 h-12 bg-gradient-to-br from-[#0058BC] to-[#3D85C6] rounded-2xl flex items-center justify-center">
              <Icon icon="mdi:bank-outline" className="text-2xl text-white" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-white">Rekening Bank</h2>
              <p className="text-white/60 text-sm">Kelola rekening untuk penarikan dana.</p>
            </div>
          </div>
        ...
        <div className="space-y-4 mb-6">
          {loading ? (
            ...
            <div className="bg-[#1A1A1A] border border-white/10 rounded-2xl p-6 text-center">
              <Icon icon="mdi:bank-off" className="text-white/60 w-8 h-8 mx-auto mb-2" />
              <h3 className="text-white font-semibold">Belum Ada Rekening</h3>
              <p className="text-white/60 text-sm">Tambahkan rekening untuk memulai penarikan.</p>
            </div>
          ) : (
            bankAccounts.map(account => (
              <div key={account.id} className="bg-gradient-to-br from-[#1A1A1A] to-[#0F0F0F] border border-white/10 rounded-3xl p-5 shadow-lg hover:shadow-xl transition-all duration-300">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 flex-1">
                    <div className="w-12 h-12 bg-gradient-to-br from-[#0058BC]/20 to-[#3D85C6]/20 rounded-2xl flex items-center justify-center border border-white/10">
                      <Icon icon="mdi:bank" className="text-2xl text-white" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="text-white font-bold text-base truncate">{account.bank_name}</h3>
                      <p className="text-white/80 font-medium text-sm">{account.account_number}</p>
                      <p className="text-white/60 text-xs truncate">{account.account_name}</p>
                    </div>
                  </div>
                  
                  <div className="flex gap-2">
                    <Link href={`/bank/edit?id=${account.id}`}>
                      <button className="p-3 rounded-xl bg-blue-500/10 text-blue-300 hover:bg-blue-500/20 hover:text-blue-200 transition-all duration-300 active:scale-95 border border-blue-500/20">
                        <Edit3 size={18} />
                      </button>
                    </Link>
                    <button 
                      onClick={() => handleDeleteClick(account)}
                      className="p-3 rounded-xl bg-red-500/10 text-red-300 hover:bg-red-500/20 hover:text-red-200 transition-all duration-300 active:scale-95 border border-red-500/20"
                    >
                      <Trash2 size={18} />
                    </button>
                  </div>
        ...
        <div className="bg-gradient-to-br from-[#1A1A1A] to-[#0F0F0F] rounded-3xl p-5 border border-white/10 shadow-2xl">
          <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
            <Icon icon="mdi:lightbulb-on-outline" className="text-yellow-400" />
            Tips Keamanan
          </h3>
      ...
      {deleteModal.show && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="bg-gradient-to-br from-[#1A1A1A] to-[#0F0F0F] rounded-3xl p-6 border border-white/10 shadow-2xl max-w-sm w-full mx-4 animate-slideUp">
            <div className="text-center">
              ...
              <p className="text-white font-semibold mb-4 text-sm">{deleteModal.account?.bank_name} - {deleteModal.account?.account_number}</p>
              <p className="text-red-300 text-xs mb-6">Tindakan ini tidak dapat dibatalkan.</p>
              
              <div className="flex gap-3">
                <button
                  onClick={handleDeleteCancel}
                  className="flex-1 bg-white/5 hover:bg-white/10 text-white font-bold py-3 rounded-xl border border-white/10 transition-all duration-300"
                >
                  Batal
                </button>
                <button
