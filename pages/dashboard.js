...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Check if popup was hidden recently
    ...
    // Show popup only if not hidden recently
  ...
  const handleClosePopup = () => {
    if (hidePopupChecked) {
      // Set localStorage to hide popup for 10 minutes
      const tenMinutesFromNow = new Date().getTime() + 10 * 60 * 1000; // 10 minutes in milliseconds
      localStorage.setItem('popupHiddenUntil', tenMinutesFromNow.toString());
    }
    setShowPopup(false);
    setHidePopupChecked(false); // Reset checkbox for next time
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-800 pb-32">
      <Head>
        <title>{applicationData?.name || 'Vla Devs'} | Dashboard</title>
        <meta name="description" content={`${applicationData?.name || 'Vla Devs'} Dashboard`} />
          ...
          <div className="flex items-center justify-between mb-6 relative z-10">
            ...
            <div className="text-center">
              <div className="w-16 h-16 bg-gradient-to-br from-purple-600 to-pink-600 rounded-2xl flex items-center justify-center shadow-xl mb-2 border border-purple-400/30">
                <Image src="/logo.png" alt="Logo" className="w-8 h-8" onError={(e) => {
                  e.target.style.display = 'none';
                  e.target.nextSibling.style.display = 'block';
                }} />
                <Icon icon="mdi:diamond" className="text-yellow-400 w-8 h-8" style={{ display: 'none' }} />
              </div>
              <p className="text-purple-200 text-xs font-medium">{applicationData?.name || 'Stoneform'}</p>
      ...
      {showPopup && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-fadeIn">
          <div className="bg-gradient-to-br from-purple-900 to-pink-800 rounded-3xl border-2 border-yellow-400/50 shadow-2xl max-w-sm w-full relative overflow-hidden animate-slideUp">
            {/* Close Button */}
            <button
            ...
            {/* Popup Content */}
            <div className="relative">
              {/* Promo Image - Changed to 1:1 aspect ratio */}
              <div className="w-full aspect-square bg-gradient-to-r from-yellow-400 to-orange-500 flex items-center justify-center relative overflow-hidden">
                <Image 
                  src="/popup.jpg" 
                ...
                <div id="popup-fallback" className="hidden flex-col items-center justify-center text-white text-center p-4 absolute inset-0 bg-gradient-to-r from-yellow-500 to-red-500">
                  ...
                  <p className="text-lg font-semibold">REBATE BONUS UNLIMITED</p>
                  <p className="text-sm mt-2">SETIAP DEPOSIT MENDAPATKAN BONUS REBATE TAMBAHAN 10%</p>
                  <p className="text-xs mt-1 font-bold">MULAI 01 JUNI - 08 JUNI 2025</p>
                </div>
              </div>
              ...
              <div className="p-4">
                ...
                <div className="flex items-center gap-2 mb-4">
                  <input
                    ...
                    className="w-4 h-4 rounded border-gray-300 text-yellow-400 focus:ring-yellow-500"
                  />
                  <label htmlFor="hidePopup" className="text-white text-sm">
