...
export default function Transactions() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-800 pb-24">
      <Head>
  <title>{applicationData?.name || 'Vla Devs'} | Riwayat Transaksi</title>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4 relative z-10">
                  <div className="bg-white/5 rounded-xl p-3 border border-white/10">
                    ...
                    <span className={`font-semibold text-sm ${tx.transaction_flow === 'debit' ? 'text-green-400' : 'text-red-400'}`}>
                      {tx.transaction_flow === 'debit' ? '+' : '-'}IDR {formatCurrency(tx.amount || 0)}
                    </span>
                  </div>
                  ...
                  {(tx.charge || 0) > 0 && (
                    <div className="bg-white/5 rounded-xl p-3 border border-white/10">
                      ...
                      <span className="text-orange-400 font-semibold text-sm">IDR {formatCurrency(tx.charge || 0)}</span>
                    </div>
                  )}
                </div>
        ...
        <div className="flex items-center justify-center gap-4 mt-6 mb-4">
          <div className="bg-gradient-to-br from-purple-800/40 to-pink-600/30 backdrop-blur-xl rounded-2xl p-4 border border-purple-400/20 shadow-xl">
            <div className="flex items-center gap-3">
              ...
              {/* Next Button - enabled only if current data length = 20 (means there might be more data) */}
              <button
                onClick={() => {
                  console.log('Next clicked, current page:', page, 'data length:', filteredTransactions.length);
                  if (filteredTransactions.length === 10) {
                    handlePageChange(page + 1);
                  }
                }}
                disabled={filteredTransactions.length < 10}
                className={`${filteredTransactions.length < 10 ? 'bg-white/5 text-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-purple-600 to-pink-600 text-white hover:from-purple-700 hover:to-pink-700 hover:scale-105 active:scale-95'} p-3 rounded-xl transition-all duration-300 shadow-lg flex items-center justify-center`}
              >
