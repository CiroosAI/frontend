...
export default function Register() {
  ...
  useEffect(() => {
    ...
    // Fetch latest settings from server for maintenance/closed_register and app name
    (async () => {
      try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result && result.success === true) {
        ...
        // Token and user storage handled in `registerUser` (sessionStorage/cookie/localStorage as appropriate).
            ...
            <form onSubmit={handleSubmit} className="space-y-6 animate-slideUp">
              ...
              <button
                ...
                className={`w-full font-bold py-4 px-6 rounded-2xl transition-all duration-300 flex items-center justify-center gap-3 shadow-lg border border-purple-400/30 relative overflow-hidden ${
                  // disable when maintenance/closed or invalid
