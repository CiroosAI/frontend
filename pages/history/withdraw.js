      ...
      <div className="max-w-sm mx-auto p-4 relative z-10">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          ...
          <div className="bg-[#1A1A1A] rounded-2xl p-4 border border-white/10">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-white/70 text-xs font-medium">Total Ditarik</span>
            </div>
        ...
        {withdrawals.length === 0 && !error ? (
          <div className="bg-[#1A1A1A] border border-white/10 rounded-2xl p-6 text-center">
            ...
            <p className="text-white/60 text-sm">Anda belum melakukan penarikan dana.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {withdrawals.map((withdrawal, index) => {
              ...
              return (
                <div key={withdrawal.id || index} className="bg-gradient-to-br from-[#1A1A1A] to-[#0F0F0F] rounded-3xl p-5 border border-white/10">
                  ...
                  <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-white font-semibold text-sm">#{withdrawal.order_id}</p>
                  ...
                  <div className="bg-white/5 rounded-xl p-4 mb-4 border border-white/10">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 bg-gradient-to-br from-[#0058BC] to-[#3D85C6] rounded-lg flex items-center justify-center">
                        <Icon icon="mdi:bank" className="text-white w-4 h-4" />
                      </div>
                      <div className="flex-1">
                        <h4 className="text-white font-semibold text-sm">{withdrawal.bank_name || 'Bank Tidak Diketahui'}</h4>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white/5 rounded-xl p-3 border border-white/10">
                      <p className="text-white/60 text-xs mb-1">Jumlah</p>
                      <p className="text-white font-bold text-sm">{formatCurrency(withdrawal.amount || 0)}</p>
                    </div>
                    
                    <div className="bg-white/5 rounded-xl p-3 border border-white/10">
                      <p className="text-white/60 text-xs mb-1">Biaya Admin</p>
                      <p className="text-red-400 font-bold text-sm">{formatCurrency(withdrawal.charge || 0)}</p>
                    </div>
                  </div>

                  {/* Final Amount */}
                  <div className={`mt-3 rounded-xl p-4 border ${statusConfig.bgColor} ${statusConfig.borderColor}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Icon icon="mdi:cash-check" className={`w-5 h-5 ${statusConfig.color}`} />
                        <span className={`text-sm font-medium ${statusConfig.color.replace('text-', 'text-').replace('-400', '-200')}`}>
                          Jumlah Diterima
                        </span>
                      </div>
        ...
        <div className="flex items-center justify-center gap-3 mt-6 mb-4">
          ...
          <div className="bg-white/5 rounded-xl px-5 py-2.5 border border-white/10 text-center">
            ...
            <p className="text-white/60 text-[10px] mt-0.5">{withdrawals.length} item</p>
          </div>

          <button
            onClick={() => { if (withdrawals.length === 10) handlePageChange(page + 1); }}
