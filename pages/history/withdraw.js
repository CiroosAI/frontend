...
export default function RiwayatWithdraw() {
  ...
  const fetchWithdrawalHistory = async () => {
    ...
    try {
      ...
      if (res.success) {
        // If data is null, treat as no withdrawals
        const fetchedWithdrawals = Array.isArray(res.data) ? res.data : [];
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-800 flex items-center justify-center">
        <div className="flex flex-col items-center">
          ...
          <span className="text-purple-200 mt-4 text-sm">Memuat riwayat penarikan...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-800 pb-24">
      <Head>
  <title>{applicationData?.name || 'Vla Devs'} | Riwayat Penarikan</title>
        ...
        {withdrawals.length === 0 && !error ? (
          ...
          <div className="space-y-4">
            {withdrawals.map((withdrawal, index) => {
              ...
              return (
                <div key={withdrawal.id || index} className="bg-gradient-to-br from-purple-800/50 to-pink-600/40 backdrop-blur-xl rounded-3xl p-6 border border-purple-400/30 shadow-2xl hover:shadow-3xl transition-all duration-300 relative overflow-hidden group">
                  ...
                  <div className="bg-gradient-to-r from-indigo-600/20 to-blue-600/20 rounded-xl p-4 mb-4 border border-indigo-400/20 relative z-10">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <h4 className="text-blue-200 font-semibold text-sm">{withdrawal.bank_name || 'Bank Tidak Diketahui'}</h4>
                  ...
                  <div className="grid grid-cols-2 gap-4 relative z-10">
                    <div className="bg-white/5 rounded-xl p-3 border border-white/10">
                      ...
                      <span className="text-cyan-400 font-semibold text-sm">IDR {formatCurrency(withdrawal.amount || 0)}</span>
                    </div>
                    ...
                    <div className="bg-white/5 rounded-xl p-3 border border-white/10">
                      ...
                      <span className="text-red-400 font-semibold text-sm">IDR {formatCurrency(withdrawal.charge || 0)}</span>
                    </div>
                  </div>
                  ...
                  <div className={`mt-4 rounded-xl p-4 border relative z-10 ${statusConfig.bgColor} ${statusConfig.borderColor}`}>
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`font-bold text-lg ${statusConfig.color}`}>
                        IDR {formatCurrency(withdrawal.final_amount || 0)}
                      </span>
                    </div>
                  </div>
                </div>
        ...
        <div className="flex items-center justify-center gap-4 mt-6 mb-4">
          <div className="bg-gradient-to-br from-purple-800/40 to-pink-600/30 backdrop-blur-xl rounded-2xl p-4 border border-purple-400/20 shadow-xl">
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => {
                  console.log('Next clicked, current page:', page, 'data length:', withdrawals.length);
                  if (withdrawals.length === 10) {
                    handlePageChange(page + 1);
                  }
                }}
                disabled={withdrawals.length < 10}
                className={`${withdrawals.length < 10 ? 'bg-white/5 text-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-purple-600 to-pink-600 text-white hover:from-purple-700 hover:to-pink-700 hover:scale-105 active:scale-95'} p-3 rounded-xl transition-all duration-300 shadow-lg flex items-center justify-center`}
              >
