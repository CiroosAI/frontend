...
export default function Team() {
  ...
  async function fetchData(cancelToken) {
    ...
    try {
      ...
      // attempt to read total members if API returns it
  ...
  useEffect(() => {
    // Ambil application data dari localStorage saat komponen mount
    ...
    if (levelChanged) {
      // store the new level marker and reset page to 1.
      prevLevelRef.current = level;
      if (page !== 1) {
        setPage(1);
        // return early; the effect will re-run when `page` updates
        return () => { cancelToken.current = true; };
      }
      // if page already 1, continue to fetch below
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-800 pb-32">
      <Head>
  <title>{applicationData?.name || 'Vla Devs'} | Tim Saya Level{level}</title>
  <meta name="description" content={`${applicationData?.name || 'Stoneform Capital'} Description`} />
        ...
        <div className={`bg-gradient-to-r ${levelColor.from}/30 ${levelColor.to}/30 backdrop-blur-xl rounded-3xl p-6 mb-6 border border-white/20 shadow-2xl`}>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="bg-white/10 rounded-2xl p-4 text-center border border-white/10">
              ...
              <div className="text-lg font-bold text-white">{teamData.totalMembers || teamData.members.length}</div>
        ...
        <div className="flex items-center justify-center gap-4 mt-6 mb-4">
          <div className="bg-gradient-to-br from-purple-800/40 to-pink-600/30 backdrop-blur-xl rounded-2xl p-4 border border-purple-400/20 shadow-xl">
            <div className="flex items-center gap-3">
              ...
              {/* Current Page Info */}
              <div className="bg-white/10 rounded-xl px-4 py-2 border border-white/20 min-w-[80px] text-center">
                <span className="text-white text-sm font-semibold">{page}</span>
                <p className="text-purple-200 text-xs mt-1">
                  {teamData.members.length} item{(teamData.members.length) !== 1 ? 's' : ''}
                </p>
              </div>

              {/* Next Button - enabled only if we haven't reached last page */}
              <button
                onClick={() => {
                  const hasMore = typeof teamData.totalMembers === 'number'
                    ? page * limit < teamData.totalMembers
                    : teamData.members.length === limit; // fallback
                ...
                className={`${(typeof teamData.totalMembers === 'number' ? page * limit >= teamData.totalMembers : teamData.members.length < limit) ? 'bg-white/5 text-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-purple-600 to-pink-600 text-white hover:from-purple-700 hover:to-pink-700 hover:scale-105 active:scale-95'} p-3 rounded-xl transition-all duration-300 shadow-lg flex items-center justify-center`}
              >
